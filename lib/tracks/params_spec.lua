...
ParamsSpec.SPEC = {
  ...
  {suffix = "mode_macro", name = "mode macro", spec = {0, 1, "lin", 0.001, 0, "", 0.001}, cmd = "modeMacro", queue = true, t1 = true},
  -- Chop/Domino warp params, migrated to the SPEC (t1) + reconciled with the shipped
  -- track-1 controlspecs (the old SPEC entries had drifted: chop_steps is the slicer's
  -- 1-64 SLICE COUNT now, chop_hold is the 0-1 GATE, chop_loop_mode has 8 modes -- so
  -- tracks 2-8 get the corrected specs too). chop_slice_len is a new entry.
  {suffix = "chop_steps", name = "chop slices", spec = {1, 64, "lin", 1, 16, "slices", 1 / 63}, cmd = "chopSteps", queue = true, t1 = true},
  {suffix = "chop_slice_len", name = "chop slice length", spec = {0.05, 32, "lin", 0.05, 1, "steps", 0.05 / 31.95}, cmd = "chopSliceLen", queue = true, t1 = true},
  -- opt_base: an OPTION on the base-value resolver (owner) -- its step p-lock is
  -- non-destructive (stored index holds, the actual bar shows the firing lock),
  -- unlike a structural selector.
  {suffix = "chop_loop_mode", name = "chop loop mode", kind = "option",
    options = {"chop", "chop rev", "loop", "loop rev", "ping pong", "ping pong rev", "runaway", "runaway rev"},
    default = 3, cmd = "chopLoopMode", offset = -1, opt_base = true, t1 = true},
  {suffix = "chop_attack", name = "chop attack", spec = {0.0001, 0.2, "lin", 0.0001, 0.002, "s", 0.0005 / 0.1999}, cmd = "chopAttack", fmt = "milliseconds", queue = true, t1 = true},
  {suffix = "chop_hold", name = "chop gate", spec = {0, 1, "lin", 0.01, 0.9, "", 0.01}, cmd = "chopHold", queue = true, t1 = true},
  {suffix = "chop_release", name = "chop release", spec = {0.0001, 0.2, "lin", 0.0001, 0.01, "s", 0.0005 / 0.1999}, cmd = "chopRelease", fmt = "milliseconds", queue = true, t1 = true},
  -- Granular (mode 4) warp params: t1 so track 1 takes the base-value resolver.
  {suffix = "grain_size", name = "grain size", spec = {0.002, 0.5, "lin", 0.001, 0.08, "s", 0.001 / 0.498}, cmd = "grainSize", fmt = "milliseconds", queue = true, t1 = true},
  {suffix = "grain_density", name = "grain density", spec = {1, 64, "lin", 1, 8, "gr/step", 1 / 63}, cmd = "grainDensity", queue = true, t1 = true},
  {suffix = "grain_jitter", name = "grain jitter", spec = {0, 0.25, "lin", 0.001, 0.01, "s", 0.001 / 0.25}, cmd = "grainJitter", fmt = "milliseconds", queue = true, t1 = true},
  {suffix = "grain_speed", name = "grain speed", spec = {0, 4, "lin", 0.01, 1, "x", 0.01 / 4}, cmd = "grainSpeed", queue = true, t1 = true},
  {suffix = "grain_speed_rand", name = "grain speed rand", spec = {0, 1, "lin", 0.01, 0, "", 0.01}, cmd = "grainSpeedRand", queue = true, t1 = true},
  {suffix = "grain_direction", name = "grain direction", spec = {0, 1, "lin", 0.01, 1, "", 0.01}, cmd = "grainDirection", queue = true, t1 = true},
  -- t1 + the ms formatter (matches track 1's old hand-registered format_ms; also
  -- makes tracks 2-8 read in ms) so the OLA warp params take the base-value resolver.
  {suffix = "wsola_window", name = "OLA window", spec = {0.005, 0.5, "lin", 0.001, 0.08, "s", 0.001 / 0.495}, cmd = "wsolaWindow", fmt = "milliseconds", queue = true, t1 = true},
  {suffix = "wsola_search", name = "OLA wander", spec = {0, 0.1, "lin", 0.001, 0.015, "s", 0.001 / 0.1}, cmd = "wsolaSearch", fmt = "milliseconds", queue = true, t1 = true},
  {suffix = "pv_window", name = "PC window", spec = {0.005, 2, "lin", 0.001, 0.2, "", 0.001 / 1.995}, cmd = "pvWindow", fmt = "milliseconds", queue = true, t1 = true},
  {suffix = "pv_dispersion", name = "PC dispersion", spec = {0, 1, "lin", 0.001, 0, "", 0.001}, cmd = "pvDispersion", queue = true, t1 = true},
  -- Harmonizer (mode 7) + Wavetable Scan (mode 8) warp params. t1 so track 1 takes
  -- the base-value resolver (batch 4: harmonizer + wavetable + spectral/formant).
  {suffix = "harm_interval", name = "harmony interval", spec = {-24, 24, "lin", 1, 7, "st", 1 / 48}, cmd = "harmInterval", queue = true, t1 = true},
  -- Two extra harmony voices (chords). 0 st = that voice OFF (owner).
  {suffix = "harm_interval2", name = "harmony interval 2", spec = {-24, 24, "lin", 1, 0, "st", 1 / 48}, cmd = "harmInterval2", queue = true, t1 = true},
  {suffix = "harm_interval3", name = "harmony interval 3", spec = {-24, 24, "lin", 1, 0, "st", 1 / 48}, cmd = "harmInterval3", queue = true, t1 = true},
  ...
  {suffix = "wt_window", name = "wavetable slices", spec = {2, 64, "lin", 1, 8, "", 1 / 62}, cmd = "wtWindow", queue = true, t1 = true},
  {suffix = "wt_cycle", name = "wavetable cycle width", spec = {16, 8192, "exp", 1, 600, "smp", 1 / 8176}, cmd = "wtCycle", queue = true, t1 = true},
  -- LFO rate: full audio range with 0.001 quant so the exponential warp-page curve
  -- keeps fine control under 1 Hz (owner). Was drifted to {0,20}; matched to the
  -- shipped hand-registered spec on migration.
  {suffix = "wt_lfo_rate", name = "wavetable LFO rate", spec = {0, 8000, "lin", 0.001, 0, "Hz", 0.001 / 8000}, cmd = "wtLfoRate", queue = true, t1 = true},
  {suffix = "wt_lfo_depth", name = "wavetable LFO depth", spec = {0, 128, "lin", 1, 64, "", 1 / 128}, cmd = "wtLfoDepth", fmt = "integer", queue = true, t1 = true},
  {suffix = "wt_lfo_shape", name = "wavetable LFO shape", kind = "option",
    options = {"sine", "tri", "saw", "s&h", "rand"}, default = 1, cmd = "wtLfoShape", offset = -1, queue = true, t1 = true},
  ...
  {suffix = "freeze_amount", name = "spectral freeze", spec = {0, 1, "lin", 0.01, 0, "", 0.01}, cmd = "freezeAmount", queue = true, t1 = true},
  {suffix = "spectral_blur", name = "spectral blur", spec = {0, 1, "lin", 0.01, 0, "", 0.01}, cmd = "spectralBlur", queue = true, t1 = true},
  {suffix = "formant_shift", name = "formant shift", spec = {-24, 24, "lin", 1, 0, "st", 1 / 48}, cmd = "formantShift", queue = true, t1 = true},
  ...
  -- non-destructive. slice_hold has NO engine cmd (Lua-read at slice-trigger time),
  ...
  -- Dry/wet mix for the Comb + Formant machines (127 = fully filtered). Ignored by
  -- the other machines (their synths accept the arg and drop it).
  {suffix = "filter_mix", name = "filter mix", spec = {0, 127, "lin", 1, 127, "", 1 / 127},
    cmd = "filterMix", xform = "amount", fmt = "env_level", queue = true, t1 = true},
  -- Filter Mix Mode: how EVERY filter machine spreads its two channels via Balance --
  -- Stereo (L/R) or Mid/Side. A setting (not p-lockable, changed only stopped + FN on
  -- the FILTER MIX page); an option like filter_machine.
  {suffix = "filter_mix_mode", name = "filter mix mode", kind = "option",
    options = {"stereo", "mid-side"}, default = 1, cmd = "filterMixMode", offset = -1, queue = true, t1 = true},
