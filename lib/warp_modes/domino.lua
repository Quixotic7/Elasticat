...
-- Page order (owner): top row Warp / SLCS / SLEN / LOOP, bottom row ATK / GATE /
-- REL / Rate (Warp + Rate are added by the coordinator in slots 1 and 8).
function Mode.source_items(Item)
  return {
    ...
    Item.item("chop_attack", "ATK", {lockable = true, env_role = "attack", min = 0.0001, max = 0.2, step = 0.0005, snaps = {0.0005, 0.001, 0.002, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2}}),
    Item.item("chop_hold", "GATE", {lockable = true, min = 0, max = 1, step = 0.01, snaps = {0, 0.25, 0.5, 0.75, 0.9, 1}}),
    Item.item("chop_release", "REL", {lockable = true, env_role = "release", min = 0.0001, max = 0.2, step = 0.0005, snaps = {0.0005, 0.001, 0.002, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2}})
  }
end
