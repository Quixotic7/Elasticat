...
elasticat.modes = {
  ...
  "spectral_freeze",  -- 9
  "formant",          -- 10
  "gstretch",         -- 11 (was 13; tape_xf/tape_ugen removed 2026-07-31)
  "gstretch2",        -- 12 (was 14)
  "chopped"           -- 13 (was 16; rubberband removed)
}
...
elasticat.param_formatters = {
  ...
  -- Seconds shown as milliseconds ("80 ms") -- the OLA/warp time params' readout,
  -- matching the old hand-registered format_ms (base-value resolver migration).
  milliseconds = format_ms,
...
-- DOMINO (mode 3) domino model: the slicer synth needs TWO regions the shared loop-
-- folded region can't express -- the RANGE (map_trim_point 0/128) as the slice AREA
-- (cut into SLCS) and the raw TRACK loop as the PLAYHEAD WINDOW (which slices fire).
-- Only pushed for a Domino track (mode 3), so no other mode pays the extra sends.
-- (Chopped, mode 16, is a plain ramp-read stretcher -- it uses the STANDARD region and
-- needs none of this.) Called from every loop/range/scrub funnel + the mode switch.
...
local function is_base_suffix(suffix)
  ...
  if entry == nil or entry.cmd == nil or entry.action ~= nil or entry.xform == "region_point" then
    return false
  end
  -- Continuous controls, PLUS option params flagged `opt_base` -- e.g. the chop LOOP
  -- mode, a per-step performance lock the owner wants NON-destructive (the stored
  -- value holds; the actual bar shows the firing lock). Structural selectors
  -- (machine / warp / filter type) have no opt_base and stay direct/destructive.
  return entry.kind == nil or (entry.kind == "option" and entry.opt_base == true)
end
...
function elasticat.params(options)
  ...
  local function apply_current_mode_params()
    ...
    if mode == 1 or mode == 2 then
      -- tape / tempo_varispeed: region-only readers.
    ...
    elseif mode == 13 then
      -- Chopped (Digitakt ramp-on-start stretch, mode 13): uses the STANDARD combined region
      -- (like tape/tempo) that the ramp sweeps, plus SLEN (grain rate) + the AHR grain
      -- gate. The new synth starts on defaults, so push the region + those params.
      tr_call(1, "loopStart", map_trim_point(params:get(ids.loop_start), 1))
      tr_call(1, "loopEnd", map_trim_point(params:get(ids.loop_end), 1))
      push("chop_slice_len", "chopSliceLen")
      push("chop_attack", "chopAttack")
    ...
    elseif mode == 11 then
      -- GStretch (varispeed reader + pitch-correct, mode 11): STRCH = grain_speed, PWIN =
      -- grain_size (window), SMTH = grain_density (shift dispersion).
      push("grain_speed", "grainSpeed")
      push("grain_size", "grainSize")
      push("grain_density", "grainDensity")
    elseif mode == 12 then
      -- GStretch2 (clock-synced, mode 12): STRCH + PWIN + SMTH (grain_density = shift dispersion).
      push("grain_speed", "grainSpeed")
      push("grain_size", "grainSize")
      push("grain_density", "grainDensity")
    elseif mode == 5 then
  ...
  -- xfade (playhead-jump crossfade, tape / tempo_varispeed, default 10ms) and pitch
  -- now register via ParamsSpec (t1) -- track 1 takes the base-value resolver. Both
  ...
  -- and now warp_rate (the slot-8 synced rate multiplier) -- registers via
  -- ParamsSpec (t1). Track 1 takes the base-value resolver (non-destructive p-locks
  -- + the actual bar) and these live under the SPEC "track 1" params group with
  -- tracks 2-8's copies. The SPEC controlspecs were reconciled to the shipped
  -- hand-registered ones (incl. the wt_lfo_rate 0-8 kHz fine curve + warp_rate's
