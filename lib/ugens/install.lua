...
-- populate. So on launch this copies ONLY the bundled prebuilt .so (+ VERSION) into
...
--
-- IMPORTANT: do NOT copy the SC class wrappers (Classes/*.sc) into Extensions.
-- norns compiles every .sc under ~/dust, and these wrappers already live in the
-- script at lib/ugens/Classes/ -- so an Extensions copy is a DUPLICATE class def
-- ("ERROR: duplicate Class found" -> the whole SC class library fails to compile
-- -> scsynth never boots -> "supercollider fail"). Only the .so belongs in
-- Extensions; the wrappers are compiled from the script dir automatically.
--
...
function Install.ensure()
  ...
  -- ONLY the .so + VERSION (see the header note: copying Classes/*.sc here would
  -- duplicate the wrappers norns already compiles from the script dir).
  local ok = run(table.concat({
    "mkdir -p " .. shq(Install.EXT),
    "rm -rf " .. shq(Install.EXT .. "/Classes"),   -- clean up any legacy duplicate
